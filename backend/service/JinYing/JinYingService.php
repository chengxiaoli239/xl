...
namespace backend\service\JinYing;
...
class JinYingService extends BaseTZService { # 冰岛时时彩登陆体系
   ...
   public static function getZjByInterval(){
    ...
    public static function downLoadCodeImgNew($uid, $tz_system_id){
        ...
        $headers = [
            ...
            'Connection: keep-alive',
            //'Cookie: '.$TzSystemsUsers->cookie,
            'Host: '.str_replace('http://', '', str_replace('https:', 'http', self::getTzSiteInfo($TzSystemsUsers->tz_system_id,'SSC_INDEX'))),
    ...
    public static function postBatchBet($qihao, $plan_id, $codes){
        ...
        foreach ($codesArrs as $key=>$tmpcodesArr){
            ...
            Tool_Common::log('bingDao_error', 'INFO', '冰岛多计划-6', ['qihao'=>$qihao, 'plan_id'=>$plan_id, 'betKey'=>$betKey, 'all_counts'=>count($codesArr), 'count'=>count($tmpcodesArr)]);
        ...
        Tool_Common::log('bet','INFO','冰岛90s批量插入记录-真实投注', $logArr);
