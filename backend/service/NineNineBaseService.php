...
class NineNineBaseService extends BaseTZService {
    ...
    public function bet($qihao, $plan_id, $code){
        ...
        if($rst['err'] == -1 OR !$rst){
            ...
            Tool_Common::log('bet','INFO','99投注记录-投注失败', $tzRst);
            return $tzRst;
        }
        ...
        //p($logArr);
        Tool_Common::log('bet','INFO','99彩票网['.$lottery_type.']插入记录-真实投注-1', $logArr);
   ...
   public static function reverseTzNowBetRecord($uid, $BetRecordId){
    ...
    public static function dwHzZuHeYL($zuhe = [2,3],$hezhis = [8,9]){
        ...
        $where = [ $field => $hezhis];
        $id = SscKjData::find()->select(['id'])->where($where)->orderBy('id DESC')->asArray()->limit(1)->one()['id'];

        $maxId = SscKjData::find()->select(['id'])->orderBy('id DESC')->asArray()->limit(1)->one()['id'];
        $times = $maxId - $id;
