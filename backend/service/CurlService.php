...
class CurlService extends BaseService{
    ...
    public static function postCurl($url,$post_data = [],$headers=[]){
        ...
        $data = curl_exec($ch);
        $errno = curl_errno( $ch );
        //if(strpos($url, 'queryMemOrderInfo') !== false)p(['url'=>$url, 'post_data'=>$post_data, 'header'=>$headers, 'rst'=>$data, 'errno'=>$errno]);
        if($errno){
    ...
    public static function getCurl302($url,$header=[]){
        ...
        if($info['http_code']==302){
            Tool_Common::log('getCurl302', 'ERR', 'getCurl 302', ['url'=>$url]);
