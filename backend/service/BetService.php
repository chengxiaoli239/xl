            ...
            try {
                ...
                foreach ($plans as $plan){
                    ...
                    if(in_array($plan->plan_type, array_merge([6, 8, 9, 14], UserSysPlans::$A_x_arise_B_y_arise_bet_B_types))){ # 6中则投 8、9遗漏多少期投
                        ...
                        Tool_Common::log('/plan/'.__FUNCTION__.'_is_bet_true', 'INFO', '是否真实下注计划', ['plan_id'=>$plan->id, 'lottery_type'=>$lottery_type, 'flag'=>$flag, 'fh'=>(boolean)(in_array($flag, [0, -1]) && $isAuto == 1)]);
                        if(in_array($flag, [0, -1]) && $isAuto == 1){
                            $is_test = 1;
                            $sn = 'istest';
                            $snid = 'istest_id';
