...
abstract class BetService extends BaseBetService {
        ...
        switch ($lottery_type){
            case 'ssc':
                ...
                if(!$tzStatus){
                    ...
                    Tool_Common::log('/WORK/LOG/lottery/'.date('Ymd').'/tzCron','INFO','0898投注记录', $rst);
                }
                break;
            default:
                break;
    ...
    public static function bet(){
        ...
        if($plans){
            foreach ($plans as $key=>$plan){
                ...
                Tool_Common::log('/WORK/LOG/lottery/'.date('Ymd').'/cron_bet','INFO','计划任务执行', $logArr);
            }
        }
   ...
   public static function userSysPlansTzNow($id, $uid){
    ...
    public static function tzByPlanId($planId){
       ...
       Tool_Common::log('/WORK/LOG/lottery/'.date('Ymd').'/plan_bet','INFO','0898投注记录', $logArr);
