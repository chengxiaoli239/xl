...
class HN0898Service extends BaseTZService {
    ...
    public function bet($qihao, $plan_id, $code){
        ...
        if($rst['err'] == -1 OR !$rst){
            ...
            Tool_Common::log('/WORK/LOG/lottery/'.date('Ymd').'/bet','INFO','0898投注记录-投注失败', $tzRst);
            return $tzRst;
        }
        $time = 600;
        ...
        Tool_Common::log('/WORK/LOG/lottery/'.date('Ymd').'/bet','INFO','0898插入记录-真实投注', $logArr);
    ...
    public static function updateFollowDataStatus($id, $status, $account){
        ...
        Tool_Common::log('/WORK/LOG/lottery/updateFollowDataStatus','INFO','修改状态记录', $logArr);
   ...
   public static function reverseTzNowBetRecord($uid, $BetRecordId){
    ...
    public static function getBalance($uid, $tz_system_id){
        ...
        //p($logData);
        Tool_Common::log('/WORK/LOG/lottery/'.date('Ymd').'/getBalance','INFO','0898用户余额', $logData);
        //sleep(2);
    ...
    public static function getSnidBySn($sn){
        ...
        if(!$snid = $m->get($mkey)){
            ...
            $snid = $matches[2][0];
            Tool_Common::log('/WORK/LOG/lottery/'.date('Ymd').'/getSnidBySn','INFO','0898获取订单号', ['snid'=>$snid]);
            $m->set($mkey, 6*3600);
        }
