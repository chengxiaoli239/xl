...
class NumService extends BaseService {
        ...
        foreach ($filter_dynamic_types as $filter_dynamic_type){
            switch ($filter_dynamic_type){
                ...
                case 7: # 过滤前200期开过号码的全转
                    $codes = NumService::getBeforeKjCodesDynamic7($plan);
                    break;
                ...
                case 10: # 过滤最近2880组(四定)，不够往前搜集 前四，与12 后4类似
                    ...
                    $codes = NumService::getBeforeKjCodesDynamic14($plan, $lottery_type, $positions=[1,2,3,4], $num=2880);
                    break;
                ...
                case 13: # 过滤最近10000期重复2次以上的直码(四定)
                    $codes = NumService::getBeforeKjCodesDynamic13($plan, $lottery_type);
                    break;
                case 14: # 过滤掉1,2,3,5最近2880组(四定)，不够往后搜集
                    $codes = NumService::getBeforeKjCodesDynamic14($plan, $lottery_type, $positions=[1,2,3,5]);
                    break;
                case 15: # 过滤掉1,2,4,5最近2880组(四定)，不够往后搜集
                    $codes = NumService::getBeforeKjCodesDynamic14($plan, $lottery_type, $positions=[1,2,4,5]);
                    break;
                case 16: # 过滤掉1,3,4,5最近2880组(四定)，不够往后搜集
                    $codes = NumService::getBeforeKjCodesDynamic14($plan, $lottery_type, $positions=[1,3,4,5]);
                    break;
                case 17: # 取前四最近8000组号码
                    $codes = NumService::getBeforeKjCodesDynamic17($plan, $lottery_type);
                    break;
                case 18: # 取后四最近8000组号码
                    $codes = NumService::getBeforeKjCodesDynamic18($plan, $lottery_type);
                    break;
                ...
                case 41: # 过滤1234前期大小或单双类型分别都不一致号码(四定)
                    $codes = NumService::getBeforeKjCodesDynamic41($plan, $filter_type=1);
                    break;
                ...
                case 44: # 过滤1234前期大小类型都不一致号码(四定)
                    $codes = NumService::getBeforeKjCodesDynamic41($plan, $filter_type=2);
                    break;
                case 45: # 过滤1234前期单双类型都不一致号码(四定)
                    $codes = NumService::getBeforeKjCodesDynamic41($plan, $filter_type=3);
                    break;
                case 46: # 过滤1234期号一致历史直码(四定)
                    $codes = NumService::getBeforeKjCodesDynamic43($plan, $positions=[1,2,3,4]);
                    break;
                case 47: # 过滤1235期号一致历史直码(四定)
                    $codes = NumService::getBeforeKjCodesDynamic43($plan, $positions=[1,2,3,5]);
                    break;
                case 48: # 过滤1245期号一致历史直码(四定)
                    $codes = NumService::getBeforeKjCodesDynamic43($plan, $positions=[1,2,4,5]);
                    break;
                case 49: # 过滤1345期号一致历史直码(四定)
                    $codes = NumService::getBeforeKjCodesDynamic43($plan, $positions=[1,3,4,5]);
                    break;
                ...
                case 57: # 过滤前50期开过号码的全转
                    $codes = NumService::getBeforeKjCodesDynamic7($plan, $positions=[1,2,3,4], $num=50);
                    break;
                case 58: # 过滤最近2345位500组(四定)，不够往后搜集
                    $codes = NumService::getBeforeKjCodesDynamic14($plan, $lottery_type, $positions=[2,3,4,5], $filterNums=500);
                    break;
                ...
                case 69: # 取个位最近x个码
                    ...
                    break;
                case 70: # 取个位最近x个码
                    $codes = NumService::getBeforeKjCodesDynamic63($plan, $positions=[1,2,3,4], $lottery_type);
                    break;
                case 71: # 过滤同单双类型+双重
                    $codes = NumService::getBeforeKjCodesDynamic64($plan, $type_field='type_ds', $positions=[1,2,3,4], $filterNums=500); #
                    break;
                case 72: # 过滤同单双类型+双重
                ...
                case 90: # 过滤[百]位号码及合分(四定)
                    $codes = NumService::getBeforeKjCodesDynamic81($plan, $positions=[2]);
                    break;
                case 91: # 过滤[十]位号码及合分(四定)
                    $codes = NumService::getBeforeKjCodesDynamic81($plan, $positions=[3]);
                    break;
                case 92: # 过滤[个]位号码及合分(四定)
                    $codes = NumService::getBeforeKjCodesDynamic81($plan, $positions=[4]);
                    break;
                ...
                case 103: # 过滤123三分离号码(四定)
                    $codes = NumService::getBeforeKjCodesDynamic102($plan, $positions=[1,2,3]);
                    break;
                case 104: # 过滤234三分离号码(四定)
                    $codes = NumService::getBeforeKjCodesDynamic102($plan, $positions=[2,3,4]);
                    break;
                case 105: # 过滤125三分离号码(四定)
                    $codes = NumService::getBeforeKjCodesDynamic102($plan, $positions=[1,2,5]);
                    break;
                ...
                case 122: # 过滤千位最近1个冷码+两大两小+对数
                    $codes = NumService::getBeforeKjCodesDynamic114($plan, $type_field='type_dx', $type_val=3, $positions=1, $type_log=2); #
                    break;
                case 123: # 过滤百位最近1个冷码+两大两小+对数
                    $codes = NumService::getBeforeKjCodesDynamic114($plan, $type_field='type_dx', $type_val=3, $positions=2, $type_log=2); #
                    break;
                case 124: # 过滤十位最近1个冷码+两大两小+对数
                    $codes = NumService::getBeforeKjCodesDynamic114($plan, $type_field='type_dx', $type_val=3, $positions=3, $type_log=2); #
                    break;
                case 125: # 过滤个位最近1个冷码+两大两小+对数
                    $codes = NumService::getBeforeKjCodesDynamic114($plan, $type_field='type_dx', $type_val=3, $positions=4, $type_log=2); #
                    break;
                case 126: # 过滤千位最近1个冷码+两大两小+对数
                    $codes = NumService::getBeforeKjCodesDynamic115($plan, $positions=1); #
                    break;
                case 127: # 过滤百位最近1个冷码+两大两小+对数
                    $codes = NumService::getBeforeKjCodesDynamic115($plan, $positions=2); #
                    break;
                case 128: # 过滤十位最近1个冷码+两大两小+对数
                    $codes = NumService::getBeforeKjCodesDynamic115($plan, $positions=3); #
                    break;
     ...
     * @return array
     */
    public static function getBeforeKjCodesDynamic7(object $plan, $positions=[1,2,3,4], $num=200){
        $playway = $plan->playway;
        $hzArr = yii\helpers\Json::decode($plan->hz_Arr);
        $lottery_type = $plan->lottery_type;
        $current_kj_qihao = $hzArr['filters']['current_kj_qihao'];
     ...
     * @return array
     */
    public static function getBeforeKjCodesDynamic10(object $plan, $lottery_type=DEFAULT_LOTTERY_TYPE, $num=2880){
        $playway = $plan->playway;
        $hzArr = yii\helpers\Json::decode($plan->hz_Arr);
        $current_kj_qihao  = $hzArr['filters']['current_kj_qihao'];
        if(empty($current_kj_qihao)){
     ...
     * @return array
     */
    private static function getBeforeKjCodesDynamic41(object $plan, $filterType=1){
        $playway = $plan->playway;
        $lottery_type = $plan->lottery_type;
     ...
     * @return array
     */
    private static function getBeforeKjCodesDynamic43(object $plan, $positions=[1,2,3,4], $limit=1000){
        $playway = $plan->playway;
        #$nextQuery = SscKjData::find()->where(['lottery_type'=>$lottery_type])->orderBy(['id'=>SORT_DESC]);
        $hzArr = yii\helpers\Json::decode($plan->hz_Arr, true);
        $current_kj_qihao = $hzArr['filters']['current_kj_qihao'];
        ...
        $historyKjDatasQuery = SscKjData::find()->select(['code_str', 'code_4n_str'=>'CONCAT('.$positions_str.')'])
            ->where($historyWhere)->groupBy(['CONCAT('.$positions_str.')'])->orderBy(['id'=>SORT_DESC]);
        $sql = $historyKjDatasQuery->createCommand()->getRawSql();  //p($sql);
        Tool_Common::log('/datas/'.__FUNCTION__, 'INFO', '过滤期号一致直码', ['positions'=>$positions, 'is_empty_c_qihao'=>$is_empty_c_qihao, 'lottery_type'=>$lottery_type, 'next_qihao'=>$next_qihao, 'plan_id'=>$plan->id, 'sql'=>$sql]);
        $historyKjDatas = $historyKjDatasQuery->asArray()->limit($limit)->all();
        $filterCodes = ArrayHelper::getColumn($historyKjDatas, 'code_4n_str');
        $filterCodesStr = implode('","', $filterCodes);
     ...
     * @return array
     */
    private static function getBeforeKjCodesDynamic61(object $plan, $positions=[1], $lottery_type=DEFAULT_LOTTERY_TYPE){
        $playway = $plan->playway;
     ...
     * @return array
     */
    private static function getBeforeKjCodesDynamic63(object $plan, $positions=[1,2,3,4], $lottery_type=DEFAULT_LOTTERY_TYPE){
        $playway = $plan->playway;
     ...
     * @param object $plan
     * @param string $type_field
     * @param string $type_ds
     * @param int $type_val
     * @param int[] $positions
     * @return array
     */
    private static function getBeforeKjCodesDynamic64(object $plan, string $type_field='type_ds', $positions=[1,2,3,4], $filterNums=1000){
        $playway = $plan->playway;
        #$nextQuery = SscKjData::find()->where(['lottery_type'=>$lottery_type])->orderBy(['id'=>SORT_DESC]);
        $hzArr = yii\helpers\Json::decode($plan->hz_Arr, true);
        $current_kj_qihao = $hzArr['filters']['current_kj_qihao'];
        ...
        if($type_field=='type_4dx'){
            $type_dd = substr($CurrentKjDatas['type_4dx'], 0, 4);
        }else{
        ...
        $NumTypes = $query->asArray()->all();
        $sql = $query->createCommand()->getRawSql();//p($sql);
        Tool_Common::log('/datas/'.__FUNCTION__, 'INFO', '过滤单双、大小+双重', ['positions'=>$positions, 'is_empty_c_qihao'=>$is_empty_c_qihao, 'lottery_type'=>$lottery_type, 'qihao'=>$qihao, 'plan_id'=>$plan->id, 'sql'=>$sql]);
     ...
     * @return array
     */
    private static function getBeforeKjCodesDynamic74(object $plan, $positions=[1,2,3,4], $c_type='type_3b', $type=1){
        $playway = $plan->playway;
    ...
    */
     ...
     * @param object $plan
     * @param int[] $positions
     * @param int dateNums 天数
     * @return array
     */
    private static function getBeforeKjCodesDynamic80(object $plan, $positions=[1], $dateNum=10){
        $hzArr = yii\helpers\Json::decode($plan->hz_Arr, true);
        $current_kj_qihao = $hzArr['filters']['current_kj_qihao'];
        $lottery_type = $plan->lottery_type;
        if(empty($current_kj_qihao)){
        ...
        $historyKjDatasQuery = SscKjData::find()->select(['code1', 'code2', 'code3', 'code4', 'code_str', 'qihao', 'type_ds'=>'code_1_2_3_4', 'type_4dx'=>'LEFT(type_4dx,4)'])
            ->where($historyWhere)->limit(1)->orderBy(['id'=>SORT_DESC]);
        $sql = $historyKjDatasQuery->createCommand()->getRawSql();//p($sql);
        Tool_Common::log('/datas/'.__FUNCTION__, 'INFO', '过滤同位置大小、单双配上期号码', ['positions'=>$positions, 'lottery_type'=>$lottery_type, 'qihao'=>$current_kj_qihao, 'plan_id'=>$plan->id, 'sql'=>$sql]);
        $historyKjData = $historyKjDatasQuery->asArray()->one();
     ...
     * @return array
     */
    private static function getBeforeKjCodesDynamic81(object $plan, $positions=[1], $num=300){
        $hzArr = yii\helpers\Json::decode($plan->hz_Arr, true);
        $current_kj_qihao = $hzArr['filters']['current_kj_qihao'];
        $lottery_type = $plan->lottery_type;
        if(empty($current_kj_qihao)){
     ...
     * @return array
     */
    private static function getBeforeKjCodesDynamic82(object $plan, $positions=[1], $num=300){
        $hzArr = yii\helpers\Json::decode($plan->hz_Arr, true);
        $current_kj_qihao = $hzArr['filters']['current_kj_qihao'];
        $lottery_type = $plan->lottery_type;
        if(empty($current_kj_qihao)){
        ...
        $historyWhere = ['AND', ['=', 'lottery_type', $lottery_type], ['=', 'qihao', $current_kj_qihao]];
        $historyKjDatasQuery = SscKjData::find()->select(['code1', 'code2', 'code3', 'code4', 'codes_hz'=>'codes_4nums_hz', 'code_str', 'qihao'])
            ->where($historyWhere)->limit(1)->orderBy(['id'=>SORT_DESC]);
        $sql = $historyKjDatasQuery->createCommand()->getRawSql();//p($sql);
        Tool_Common::log('/datas/'.__FUNCTION__, 'INFO', '过滤同位置大小、单双配上期号码', ['positions'=>$positions, 'lottery_type'=>$lottery_type, 'qihao'=>$current_kj_qihao, 'plan_id'=>$plan->id, 'sql'=>$sql]);
        $historyKjData = $historyKjDatasQuery->asArray()->one();
    ...
    private static function getBeforeKjCodesDynamic83(object $plan, $c_type=1){
        ...
        $historyKjDatasQuery = SscKjData::find()->select(['code1', 'code2', 'code3', 'code4', 'code_str', 'qihao'])
            ->where($historyWhere)->limit(1)->orderBy(['id'=>SORT_DESC]);
        $sql = $historyKjDatasQuery->createCommand()->getRawSql();//p($sql);
        Tool_Common::log('/datas/'.__FUNCTION__, 'INFO', '过滤上期每两个号码及对数', ['c_type'=>$c_type,'lottery_type'=>$lottery_type, 'qihao'=>$current_kj_qihao, 'plan_id'=>$plan->id, 'sql'=>$sql]);
        $historyKjData = $historyKjDatasQuery->asArray()->one();
    ...
    private static function getBeforeKjCodesDynamic112(object $plan, $type_field='type_4ds'){
        ...
        $CurrentKjDatas = SscKjData::find()->select(['code1', 'code2', 'code3', 'code4', 'code_str', 'qihao', 'type_ds'=>'code_1_2_3_4', 'type_4dx'=>'LEFT(type_4dx,4)'])
            ->where(['lottery_type'=>$lottery_type, 'qihao'=>$current_kj_qihao])->orderBy(['id'=>SORT_DESC])->asArray()->one();
        if($type_field=='type_4dx'){
            $filterCode1 = NumService::getDxTypeByCode($CurrentKjDatas['code1']);
            $filterCode2 = NumService::getDxTypeByCode($CurrentKjDatas['code2']);
            $filterCode3 = NumService::getDxTypeByCode($CurrentKjDatas['code3']);
            $filterCode4 = NumService::getDxTypeByCode($CurrentKjDatas['code4']);
        }else{
            $filterCode1 = NumService::getDxTypeFanByCode($CurrentKjDatas['code1']);
            $filterCode2 = NumService::getDxTypeFanByCode($CurrentKjDatas['code2']);
     ...
     * @return array
     */
    private static function getBeforeKjCodesDynamic115(object $plan, $pos=1){
        $playway = $plan->playway;
        ...
        $latelyCode = NumService::getPosLatelyCode($pos, $num=9, $lottery_type);
        $filterCodes = array_diff(\backend\service\NumService::$ALL_CODES, $latelyCode); # 过滤1冷码
        ...
        $NumTypes = $query->asArray()->all();
        $sql = $query->createCommand()->getRawSql();//p($sql, 0);
        Tool_Common::log('/datas/'.__FUNCTION__, 'INFO', '过滤冷码+合分', ['pos'=>$pos, 'is_empty_c_qihao'=>$is_empty_c_qihao, 'lottery_type'=>$lottery_type, 'qihao'=>$qihao, 'plan_id'=>$plan->id, 'latelyCode'=>$latelyCode, 'filterCodes'=>$filterCodes, 'sql'=>$sql]);
        #p(['count'=>count($NumTypes), 'sql'=>$sql, 'NumTypes'=>$NumTypes]);
        $codes = ArrayHelper::getColumn($NumTypes, 'code');
     ...
     * @return int[]
     */
    public static function getDoubleTypeByCode($num){
        if(in_array($num, NumService::$MIN_CODES) && in_array($num, NumService::$SINGLE_CODES)){
            # 小单
            $codes = NumService::DOUBLE_TYPE_XD;
        }elseif (in_array($num, NumService::$MIN_CODES) && in_array($num, NumService::$DOUBLE_CODES)){
            # 小双
            $codes = NumService::DOUBLE_TYPE_XS;
        }elseif (in_array($num, NumService::$MAX_CODES) && in_array($num, NumService::$SINGLE_CODES)){
            # 大单
            $codes = NumService::DOUBLE_TYPE_DD;
     ...
     * @return int[]
     */
    public static function getDsTypeFanByCode($num){
