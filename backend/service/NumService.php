...
class NumService extends BaseService {
    ...
    public static function getDescByKuaixuan($hz_Arr, $plan_id=''){
        ...
        if($UserSysPlans && in_array($UserSysPlans->plan_type, [14])){
            $desc .= '【条件:'.$hz_Arr['area_all_qishus'].'漏'.$hz_Arr['area_yl_qishus'].'期';

            $desc .= ($hz_Arr['areaBetStatus']==0)? ',当前'.($hz_Arr['area_all_qishus']-$hz_Arr['area_yl_qishus']).'期' : '';

            $desc .= '|止盈:'.$hz_Arr['area_profits'].'止损:'.$hz_Arr['area_loss'];

            $desc .= isset($hz_Arr['current_area_profits']) ? '当前:'.number_format($hz_Arr['current_area_profits'], 2) : ''; # 当前遗漏
