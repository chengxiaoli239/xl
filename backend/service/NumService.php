...
class NumService extends BaseService {
        ...
        foreach ($filter_dynamic_types as $filter_dynamic_type){
            switch ($filter_dynamic_type){
                ...
                case 28: # 过滤期号尾号一致历史直码(四定) - 1234
                    $codes = NumService::getBeforeKjCodesDynamic28($plan, $positions=[1,2,3,4], 3000);
                    break;
                ...
                case 97: # 过滤上期每两个号码及对数(四定)
                    $codes = NumService::getBeforeKjCodesDynamic83($plan, $dateNum=0);
                    break;
                case 98: # 过滤昨日同期每两个号码及对数(四定)
                    $codes = NumService::getBeforeKjCodesDynamic83($plan, $dateNum=1);
                    break;
                ...
                case 142: # 过滤上期每两个号码及双重(四定)
                    $codes = NumService::getBeforeKjCodesDynamic83($plan, $dateNum=0, $d_type=2);
                    break;
    ...
    */
     ...
     * @return array
     */
    private static function getBeforeKjCodesDynamic117(object $plan, $pos=1){
        $playway = $plan->playway;
        $hzArr = Json::decode($plan->hz_Arr, true);
        $current_kj_qihao = $hzArr['filters']['current_kj_qihao'];
        $lottery_type = $plan->lottery_type;
