...
<strong><font color="blue">8、A出x次+B出y次投B（按A判断）：</font></strong>A组号码连续出现x次后，再按B组连续出现y次作为触发条件，y可填0；A、B允许部分重叠，判断时A优先。满足条件后下一期实际投注B，之后每期只判断A：A中则继续投B，A不中则等待下一次满足条件；不判断B是否中奖来决定策略。倍数梯度不填时始终使用基础倍数；填写梯度（如1-3-5）时，每次实际投注后推进下一档，A中下一期直接投下一档，A不中则等待条件满足后使用下一档，最后一档后回到第一档循环，重复倍数按填写位置执行。
