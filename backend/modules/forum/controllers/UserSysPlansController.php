...
class UserSysPlansController extends BaseController
{
    ...
    public function actionUpdate($id)
    {
        ...
        }elseif (in_array($model->tz_type, [18, 20, 25, 29, 30, 31, 32, 33])){
            $hz_Arr_Data = json_decode($model->hz_Arr, true);
            //p($hz_Arr_Data);
            foreach ($hz_Arr_Data as $key=>$val){
                if(in_array($key, ['hz', 'p1', 'p2', 'p3', 'p4', 'p5', 'status_val', 'type_4ds', 'code1', 'code2', 'arise', 'type_4d', 'type_4s', 'hefen', 'no_fix_hefen', 'arise_in', 'xhenfen','singles_key'])){
                    $model->$key = $val;
