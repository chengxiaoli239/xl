...
class MessageOperateService  extends BaseService
{
    ...
    private function matchOtherOperate($text, $message=[]): array
    {
        ...
        switch ($text){
            ...
            default:
                ...
                Tool_Common::log('/bet_aozhou5/'.__FUNCTION__, 'INFO', '盘口状态检测', ['lottery_type'=>$this->lottery_type, 'status'=>$status, 'statusTxt'=>LotteryBet::STATUS_OPTIONS[$status], 'switch'=>$switch]);
    ...
    public function receive($message, $token): array
    {
        ...
        if($this->tzSystemUsers->is_local_bet == BetsBackend::BET_TYPE_SERVER_API){
