...
class MethodMatchService extends CommonBaseService
{
    ...
    public static function matchZhiZuOrZuSanOrZuLiuXMa(string $text='', array &$codes=[], &$count=0, array $singleArr=[]): array
    {
        ...
        //p([$text, $matcheCodeText, $matches, $codes, $singleArr]);
        if(empty($codes)){
            throw_info('匹配组选号码为空22');
        }
        $methodArr = [
            'methodArrZhi' => [],
            'methodArr3' => [],
                ...
                case strpos($text, '直') !== false OR strpos($text, '单选') !== false OR strpos($text, '单') !== false:
                    ...
                    switch (true){
                        ...
                        case preg_match_all('/直\s*(\d+)元/u', $text, $matches)&& strpos($text, '倍')===false:
                            ...
                            break;
                        case (preg_match_all('/(['.MethodMatchService::CN_SINGLE_TEXT.']{1,3})\s*直/u', $text, $matches)&& strpos($text, '倍')===false && strpos($text, '元')===false):
                        case preg_match_all('/((\d+){1,3})\s*直/u', $text, $matches)&& strpos($text, '倍')===false && strpos($text, '元')===false:
                        ...
                        case preg_match('/((\d+){1,3})\s*单/u', $text, $matches):
