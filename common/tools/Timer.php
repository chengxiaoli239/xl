...
class Timer
{
     ...
     * 在需要计时的地方做标记
     */
    public static function markTag($mark){
        if(Yii::$app->params['time_diagnosis'] == false)
            return;
        self::getTimer()->mark($mark);
    }
    ...
    public static function getTimer(){
        if(self::$timer == null) {
            ...
            self::$filePath = Yii::$app->getRuntimePath() . '/logs/php-time.log';
            $logPath = dirname(self::$filePath);
            if (!is_dir($logPath)) {
    ...
    public static function logAllIntervalToFile(){
        if(Yii::$app->params['time_diagnosis'] == false)
            return;
        $arr = self::getTimer()->report;
        if(count($arr) <= 1)
            return;
        ...
        foreach(Util::checkArray($allKeys) as $item){
            $now = $arr[$item];
            $now['name'] = $item;
            if($last == null) {
                $last = $now;
            ...
            $time = $now['time'] - $last['time'];
            if($time < 0.0001)
                $time = 0;
            $message .= '    '. $last['name'].' -> ' . $now['name']. ' : '.$time . PHP_EOL;
            $last = $now;
        }

        $total = $arr[$allKeys[count($allKeys) - 1]]['time'] - $arr[$allKeys[0]]['time'] ;
    ...
    protected function logReportRecord($file, $mark, $report)
    {
        ...
        $memory_unit = $this->memory_unit;
        $message = '------------------------------------------' . PHP_EOL;
        $message .= date('Y-m-d H:i:s') ."  mark:" . $mark  .PHP_EOL
            . "time:" . $report['time'] . 's' . PHP_EOL
            . "memory_real:" . ($report['memory_real'] / $memory_rate) . $memory_unit . PHP_EOL
            . "memory_emalloc:" . ($report['memory_emalloc'] / $memory_rate) . $memory_unit . PHP_EOL
            . "memory_peak_real:" . ($report['memory_peak_real'] / $memory_rate) . $memory_unit . PHP_EOL
