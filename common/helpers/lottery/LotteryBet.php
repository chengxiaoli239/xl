...
class LotteryBet
{
    ...
    public array $schedule = [
        ...
        28 => [
            ...
            'closeOffset' => -30, // 封盘时间是第4分钟的前30秒，例如 3分30秒
            'open' => 30,  // 开盘时间是开奖后的50秒
        ],
        // 更多彩种配置...
    ];
    public array $schedule1 = [
        5 => [
            'close' => -30,  // 封盘前30秒
            'draw' => 0,     // 抓取开奖时间开始
            'open' => 50,    // 开盘时间在开奖后的50秒
        ],
        28 => [
            'close' => -90,  // 封盘前90秒
            'draw' => -60,   // 抓取开奖时间开始
            'open' => 10,    // 开盘时间在开奖后的50秒
        ],
        // ... 可以为其他彩种继续添加配置
    ];
    ...
    public function checkLotteryStatus($lotteryType, $currentTime=''): int
    {
        ...
        if(($closeStart<=$currentTime && $currentTime<=$closeEnd)) {
            ...
            return LotteryBet::STATUS_START; # 当前为封盘状态
