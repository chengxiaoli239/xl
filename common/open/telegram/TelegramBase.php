<?php
namespace common\open\telegram;

use common\models\open\telegram\RequestLog;
use common\open\OpenBase;
use common\open\thirdD\api\SiteOauthApi;
use common\open\thirdD\api\SxThirdDBase;
...
class TelegramBase extends OpenBase
{
    /**
     * @var string api请求地址
     */
    ...
    protected function request(string $method, $apiMethod, array $params = [], array $headers = [], array $options = []): ?array
    {
        ...
        try {
            ...
            $status = RequestLog::REQUEST_STATUS_SUCCESS;
            return $result;
        } catch(\Exception $e) {
            $code = $e->getCode();
            ...
            $status = RequestLog::REQUEST_STATUS_FAIL;
            ...
            $status = $status ?? RequestLog::REQUEST_STATUS_FAIL;
            ...
            RequestLog::find()->createCommand()->insert(RequestLog::tableName(), $logData)->execute();
        }
    }
